...
const { checkValidation, MulterParser } = require('../../_helpers')
...
const apiAuthLogin = async function(req, res) {
  ...
  // Find the user
  let user = await User.findOne({ where: { email }, order: [[User.UserImages, 'id', 'ASC']], include: User.UserImages })
  ...
  if (!validPassword) return res.status(401).json({ message: 'Credentials incorrect' })
...
module.exports = [MulterParser.none(), validation, checkValidation, apiAuthLogin]